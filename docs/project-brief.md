# GadgetMoTo Project Brief

## Brand

- Name: GadgetMoTo
- Slogan: Your Next Upgrade, Mo ’To.
- Supporting line: Phones and tablets for every lifestyle, budget, and way to pay.
- Store pickup location: LOT 1 DON PLACEDO CAMPUS AVE BRGY SABANG, Dasmariñas, Philippines, 4114
- Primary contact: Facebook Messenger
- Facebook page: https://www.facebook.com/profile.php?id=100063905416187

## Visual direction

- Futuristic
- Modern
- Minimal
- Premium
- Light blue and white
- Mobile-first
- Awwwards-inspired interactions
- GSAP hero animation will be added in a later checkpoint

## Brand colors

- GadgetMoTo Blue: #4C91C8
- Graphite: #37363B
- Ice Blue: #F3F9FE
- Bright Blue: #247FE5
- Deep Ink: #171A20
- White: #FFFFFF

## Planned typography

- Headings: Space Grotesk
- Body and interface: Inter

## Store requirements

- Phones and tablets only
- All launch products treated as brand new
- Guest checkout without customer accounts
- One physical branch
- Nationwide courier delivery
- Same-day delivery where available
- Store pickup
- Real-time product-variant inventory
- In-stock, low-stock, preorder, and unavailable statuses
- Unavailable products use a Message Us button
- Facebook Messenger as the main contact channel
- Product comparison
- Live search suggestions
- Product thumbnails in search
- Trending searches
- Staff-managed products and homepage content
- Price-drop alerts
- Product images will initially use placeholders and be replaced later

## Launch payments

- Maya online payment
- Cash on store pickup
- Manual bank or e-wallet transfer
- No cash on delivery
- VAT calculated separately
- Do not implement or assume a VAT rate until confirmed

## Initial prototype products

1. Xiaomi 17 Ultra 5G Leica Kit — 16GB/512GB — ₱84,990
2. Apple iPhone 17 — 256GB — ₱57,990
3. POCO F8 Ultra — 16GB/512GB — ₱46,990
4. Redmi Note 15 Pro Plus 5G — 12GB/512GB — ₱27,990
5. Redmi Turbo 5 — 12GB/256GB — ₱20,990
6. Infinix Note 60 Pro 5G — 8GB RAM + 8GB Extended / 256GB — ₱19,990
7. TECNO Camon 50 — 8GB RAM + 8GB Extended / 256GB — ₱13,490
8. POCO C85 — 8GB/256GB — ₱7,990
9. POCO Pad X1 — 8GB/512GB — ₱23,990
10. Xiaomi Pad 8 — 8GB/128GB — ₱19,990
11. Redmi Pad 2 Pro 5G — 8GB/256GB — ₱18,990
12. TECNO Mega Pad Pro — 8GB/256GB — ₱13,990
