...
import { getAllProducts, type PrototypeProduct } from "@/data/prototype-products";

const storageKey = "gadgetmoto:compare:v1";
const maxProducts = 3;
const allProducts = getAllProducts();
const validSlugs = new Set(allProducts.map((product) => product.slug));
...
function applySelectionAction(current: string[], action: SelectionAction) {
...
export function ComparisonProvider({ children }: { children: ReactNode }) {
  ...
  const dispatchSelection = (action: SelectionAction) => {
    if (!hydratedRef.current) pendingActionsRef.current.push(action);
    setSelectedSlugs((current) => applySelectionAction(current, action));
  };
  ...
  useEffect(() => {
    const restore = window.setTimeout(() => {
      try {
        ...
        setSelectedSlugs(pendingActionsRef.current.reduce(applySelectionAction, validated));
      } catch {
  ...
  const value = useMemo<ComparisonContextValue>(() => {
    ...
    return { selectedSlugs, selectedProducts: selectedSlugs.flatMap((slug) => { const product = allProducts.find((item) => item.slug === slug); return product ? [product] : []; }), selectionCount: selectedSlugs.length, canAddMore: selectedSlugs.length < maxProducts, feedback, isSelected, addProduct, removeProduct, toggleProduct, clearProducts: () => { dispatchSelection({ type: "clear" }); setFeedback(""); } };
  }, [feedback, selectedSlugs]);
