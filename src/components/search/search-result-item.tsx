...
export function SearchResultItem({
  ...
  return (
    <Link
      aria-label={`View ${product.name}, ${product.variant}, ${priceFormatter.format(product.currentPrice)}`}
      ...
      id={`global-search-result-${product.slug}`}
      onClick={onNavigate}
      onMouseEnter={onHover}
      role="option"
    >
      <span className="global-search-result__art">
        <ProductArtwork product={product} sizes="5rem" />
      </span>
      <span className="global-search-result__content">
        <span>
          ...
          {product.badge === "new" ? <em>New</em> : null}
        </span>
